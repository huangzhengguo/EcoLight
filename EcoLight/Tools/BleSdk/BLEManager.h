...
/**
 *  OAD升级的类型 1 = 2541 a,b版升级, 2 = 2541大容量升级, 3 = 2640片内升级, 4 = 2640片外升级 4 = 2640片外升级
 */
@property (nonatomic, assign) NSInteger oadStyle;
 ...
 */
- (void)startOADFileWithFileName:(NSString *)fileName filePath:(NSInteger)path;
...
- (void)configure2640R2Profile:(CBPeripheral*)cb;

#pragma mark - 2640 片外 OAD 升级
/**
 * path path = 1 升级文件是放在工程里面 path = 2 升级文件放在沙盒目录的Documents文件夹下面
 */
- (void)startOADFileWithFileName:(NSString *)fileName peripheral:(CBPeripheral *)cb filePath:(NSInteger)path progressBlock:(OAD_PROGRESS_Block)block;

#pragma mark - 2640 R2 OAD 升级
- (void)R2_Off_Chip_OadStartOADFileWithFileName:(NSString *)fileName peripheral:(CBPeripheral *)cb filePath:(NSInteger)path;

#pragma mark - 2640 R1 OAD 升级
- (void)R1_Off_Chip_OadStartOADFileWithFileName:(NSString *)fileName peripheral:(CBPeripheral *)cb filePath:(NSInteger)path;
